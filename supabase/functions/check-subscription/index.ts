...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      ...
      // Also mirror to profiles for existing UI logic
    ...
    if (hasActiveSub) {
      ...
      logStep("Active subscription found", { subscriptionId, subscriptionEnd, amount, subscriptionTier });
    } else {
    ...
    // Mirror to profiles for existing UI logic
    await supabaseClient.from('profiles')
      .update({
